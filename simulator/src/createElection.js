...
const createRegions = async (electionContract, regionData) => {
  ...
  return await new Promise(async resolve => {
    for(let i = 0; i < regionData.length; i++) {
      ...
      const receipt2 = await electionContract.methods
...
const createElection = async (web3, electionContract, partyData, regionData) => {
  ...
  var parties = [];
	for(let i = 0; i < partyData.length; i++) {
		...
		await electionContract.methods
      .addParty(partyID, party.name, party.list)
      .send({from: ownerAddress});
  ...
  for(let i = 0; i < regions.length; i++) {
    for(let j = 0; j < process.env.SIMULATION_NUMBER_OF_MANAGERS_PER_REGION; j++) {
      ...
      console.log(`Manager account '${managerAddress}' created for region ${regions[i].name}`);
    };
		await electionContract.methods
      .addManagerListToRegion(regions[i].id, regions[i].managers)
      .send({from: ownerAddress});
  ...
  for(let i = 0; i < regions.length; i++) {
    for(let j = 0; j < regions[i].census.length; j++) {
      ...
      regions[i].voters.push(voterAddress);
			console.log(`Voter account '${voterAddress}' created for region ${regions[i].name}`);
    };
    await electionContract.methods
      .authoriseVoterList(regions[i].id, regions[i].voters)
      .send({from: ownerAddress});
